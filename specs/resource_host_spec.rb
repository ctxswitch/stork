...
describe "Stork::Resource::Host" do
  it "must create a host" do
    Stork::Resource::Host.new(configuration, "example.org").must_be_instance_of Stork::Resource::Host
  end

  it "must respond to the name method" do
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :name
  end

  it "must respond to the layout accessors" do
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :layout
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :layout=
  end

  it "must respond to the template accessors" do
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :template
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :template=
  end

  it "must respond to the chef accessors" do
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :chef
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :chef=
  end

  it "must respond to the pxemac accessors" do
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :pxemac
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :pxemac=
  end

  it "must respond to the pre_snippets accessors" do
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :pre_snippets
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :pre_snippets=
  end

  it "must respond to the post_snippets accessors" do
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :post_snippets
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :post_snippets=
  end

  it "must respond to the interfaces accessors" do
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :interfaces
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :interfaces=
  end

  it "must respond to the distro accessors" do
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :distro
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :distro=
  end

  it "must respond to the run_list accessors" do
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :run_list
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :run_list=
  end

  it "must respond to the repos accessors" do
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :repos
    Stork::Resource::Host.new(configuration, "example.org").must_respond_to :repos=
  end
