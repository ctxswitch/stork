...
network --device=eth0 --bootproto=static --ip=192.168.1.100 --netmask=255.255.255.0 --gateway=192.168.1.1 --nameserver=192.168.1.253,192.168.1.252 --onboot=yes
...
/usr/bin/curl http://localhost/notify/example.com/installed
...
start() {
  $exec -j /etc/chef/first-boot.json -N example.com
  retval=$?
  $chk chef-client on
...
knife client reregister example.com -f /etc/chef/client.pem --config /root/.chef/knife.rb
echo "Getting rid of the root pem"
rm /root/.chef/root.pem

cat > /etc/chef/first-boot.json << 'EOF'
{"run_list":["recipe[sudo]","recipe[authentication]","recipe[nagios]","recipe[apache]"]}
EOF
